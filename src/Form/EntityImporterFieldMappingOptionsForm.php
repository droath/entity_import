...
/**
 */
class EntityImporterFieldMappingOptionsForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $entity_importer_type = NULL) {
    ...
    for ($i = 0; $i < $items_count; $i++) {
      ...
      $form['unique_identifiers']['items'][$i]['identifier_name'] = [
        ...
        '#size' => 10,
        '#required' => TRUE,
        '#default_value' => isset($items['identifier_name']) ? $items['identifier_name'] : NULL,
      ];
      ...
      if (isset($reference_type) && !empty($reference_type)) {
        ...
        $form['unique_identifiers']['items'][$i]['identifier_type'] = [
          ...
          '#required' => TRUE,
          '#empty_option' => $this->t('- Select -'),
          '#default_value' => (isset($identifier_type) && !empty($options[$identifier_type]))
            ? $identifier_type
            : NULL,
        ];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (isset($identifiers['items'])
      ...
      foreach ($identifiers['items'] as $delta => $item) {
        if (!is_int($delta) || empty($item['identifier_name'])) {
          continue;
        }
 ...
 */
   ...
   */
  protected function updateUniqueIdentifiers($importer_type_id, array $values) {
  ...
  protected function getConfig($importer_type_id, $editable = FALSE) {
    ...
    $config_name = "entity_import.field_mapping.options.{$importer_type_id}";
